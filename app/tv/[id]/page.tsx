...
import { Movie, TvDetails } from "@/typings";
import { Play } from "lucide-react";
import Image from "next/image";
import { useEffect, useState } from "react";
import ReactPlayer from "react-player";

function TvIdpage({ params: { id } }: { params: { id: string } }) {
  ...
  const [movie, setMovie] = useState<TvDetails>();
  const [MoreLikeThis, setMoreLikeThis] = useState<Movie[]>([]);
  const [getRecommended, setgetRecommended] = useState<Movie[]>([]);
  useEffect(() => {
    getTvDetails(parseInt(id)).then((res) => setMovie(res));
    getRecommendationsTv(parseInt(id)).then((res) => setMoreLikeThis(res));
    getMoreLikeThisTv(parseInt(id)).then((res) => setgetRecommended(res));
  }, [id]);

  if (movie === undefined) return null;

  const PlayMovie = (id: number) => {
    getPlayTvTrailer(id).then((res) => {
      res.find((item) => {
        if (
          item.official === true &&
          item.site === "YouTube" &&
  ...
  return (
    ...
    `}
        ...
        {officalTrailer === undefined ? (
          <>
            ...
            <div className="hidden lg:inline absolute mt-0 top-0 pt-[13rem]  left-0  bg-transparent z-[300] h-full w-full bg-gradient-to-r from-gray-900/50 via-transparent to-transparent p-10 space-y-5 text-white  pl-20">
              <h2 className="text-5xl font-bold max-w-xl ">{movie.name}</h2>
      ...
      `}
              ...
              {movie?.genres?.map((item) => {
                return (
                  <span
                    key={item.id}
                    className="px-2 mx-2 py-1 text-xs font-medium text-white bg-slate-400/50 rounded-md"
                  >
                    {item.name}
                  </span>
                );
      ...
      <div className={`w-full pb-[20px]  bg-gray-900/90  lg:p-20  lg:hidden `}>
       ...
       bg-slate-400/50 px-1 rounded-md
        `}
          ...
          <div
            className={`flex space-x-2 items-center justify-center flex-wrap`}
          >
            {movie?.genres?.map((item) => {
              return (
                <span
        ...
        <h1
          className={`text-2xl font-bold px-10 py-4 text-white w-full 
            
          ${getRecommended.length === 0 ? "hidden" : ` block`}
             `}
        >
          More Like This
        </h1>
        ...
        >
          {getRecommended.map((item, index) => (
            <MovieCard key={item.id} movie={item} tv index={index} />
          ))}
        </div>
        <h1 className={`text-2xl font-bold px-10 py-4 text-white w-full `}>
          Suggestions
        </h1>
        <div
      ...
      <h2 className="text-xl font-bold px-10 pt-4 ml-7 hidden lg:inline">
        {"Suggestions"}
      </h2>
